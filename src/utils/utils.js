...
export const sendIfttt=(value1=null, value2=null, value3=null)=>{
    return axios.post(`https://maker.ifttt.com/trigger/${config.iftttEvent}/with/key/${config.iftttKey}`,
    {value1 , value2, value3}) 
    .then(data=>{
      if(data.data!==`Congratulations! You've fired the cbb event`) throw Error('IFTTT EVENT sent failed')
    })
}
...
export const plus=(a, b)=>{
    ...
    return (x.plus(y)).decimalPlaces(10).toNumber()  
}

export const minus=(a, b)=>{
    const x = new BigNumber(parseFloat(a).toFixed(10))
    const y = new BigNumber(parseFloat(b).toFixed(10))
    return (x.minus(y)).decimalPlaces(10).toNumber()
}

export const multi=(a, b)=>{
    const x = new BigNumber(parseFloat(a).toFixed(10))
    const y = new BigNumber(parseFloat(b).toFixed(10))
    return (x.multipliedBy(y)).decimalPlaces(10).toNumber()
}

export const div = (a, b) => {
        const x = new BigNumber(parseFloat(a).toFixed(10))
        const y = new BigNumber(parseFloat(b).toFixed(10))
        return (x.dividedBy(y)).decimalPlaces(10).toNumber()
