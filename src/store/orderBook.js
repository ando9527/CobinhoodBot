...
export function orderBookReducer(state: OrderBook = null, action: AllOrderBookActions ) {
  switch (action.type) {
    ...
    case UPDATE_ORDER_BOOK: {
      logger.debug(`[Redux][orderBookReducer] UPDATE_ORDER_BOOK-payload ${JSON.stringify(action.payload)}`)
...
export const ordersMaker=({orders, uOrders}: {orders: Array<Order>, uOrders: Array<Order>}): Array<Order>=>{
  let newOrders = []
  let flag = null
...
export const orderBookMaker=({state, orderBook}:{state: OrderBook, orderBook: OrderBook}): OrderBook=>{ 
